...
def find_tfidf(query):
    print("in tf idf*"*5,query)
    ...
    news = pd.read_json('static/preprocessedData.json')
    vocabulary, tfidf, tfidf_normalized = createTFIDF(news['lemmatized'])

    return cosine_similarity(query, vocabulary, tfidf, tfidf_normalized, news, 10)

#print(find_tfidf("Windows"))